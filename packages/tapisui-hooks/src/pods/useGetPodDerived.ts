...
const useGetPodDerived = (
  ...
  const result = useQuery<Pods.PodResponse, Error>(
    [QueryKeys.getPodDerived, params, accessToken],
    // Default to no token. This will generate a 403 when calling the list function
    // which is expected behavior for not having a token
    () => API.getPodDerived(params, basePath, accessToken?.access_token ?? ''),
    {
  ...
  const invalidate = () => {
    queryClient.invalidateQueries([QueryKeys.getPodDerived]);
  };

  return { ...result, invalidate };
};

export default useGetPodDerived;
