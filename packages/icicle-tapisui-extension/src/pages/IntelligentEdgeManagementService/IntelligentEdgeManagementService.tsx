...
const IEMS_ORIGIN = 'https://edgecontrolplane.pods.icicleai.tapis.io';
const AUTH_REQUEST_TYPE = 'iems:portal-auth:request';
const AUTH_RESPONSE_TYPE = 'iems:portal-auth:response';
...
export const IntelligentEdgeManagementService: Component = ({
  accessToken,
}) => {
  ...
  React.useEffect(() => {
    ...
    const handlePortalAuthRequest = (event: MessageEvent) => {
      const iemsWindow = iframeRef.current?.contentWindow;
      if (
        event.origin !== IEMS_ORIGIN ||
        !iemsWindow ||
        event.source !== iemsWindow ||
    ...
    >
      {accessToken ? (
        <iframe
          ...
          src={IEMS_ORIGIN}
          title="IntelligentEdgeManagementService"
        />
      ) : (
...
export default IntelligentEdgeManagementService;
