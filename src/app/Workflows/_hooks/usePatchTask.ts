...
import { TaskUpdateContext, type TaskUpdateContextProps } from "app/Workflows/_context";
...
const usePatchTask = <T>(): UsePatchTask<T> => {
  const context = useContext(TaskUpdateContext) as TaskUpdateContextProps<T> | null;
  if (context === null) {
    throw new Error('usePatchTask must be used within a TaskUpdateContextProvider');
  }
  ...
  const modifiedUsePatchHook: Omit<ReturnType<typeof Hooks.Tasks.usePatch>, "patch"> & { patch?: typeof patch } = { ...usePatchHook }
