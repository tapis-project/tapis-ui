import React from 'react';
import { Position, NodeProps } from '@xyflow/react';
import styles from './ConditionsNode.module.scss';
import { StandardHandle } from '../../Handles';
import { Workflows } from '@tapis/tapis-typescript';
import { Edit, Delete } from '@mui/icons-material';
import { useHistory } from 'react-router-dom';

type NodeType = {
  task: Workflows.Task;
  groupId: string;
  pipelineId: string;
};


const conditionsImageSrc = "data:image/jpeg;base64,"

const ConditionsNode: React.FC<NodeProps> = ({ data }) => {
  const { task } = data as NodeType;
  return (
    <>
      <StandardHandle
        id={`conditions-target-handle-${task.id}`}
        type="source"
        position={Position.Left}
      />
      <div className={styles['node']}>
        <div className={styles['header']}>
          <img src={conditionsImageSrc} className={styles['header-img']} />
          <span className={styles['title']}>Conditions</span>
        </div>
        <div className={styles['body']}>
          <i className={styles['description']}>
            Conditionals that control task execution
          </i>
        </div>
        <div>
          
        </div>
      </div>
      <StandardHandle
        id={`conditions-source-handle-${task.id}`}
        type="source"
        position={Position.Right}
      />
    </>
  );
};

export default ConditionsNode;
