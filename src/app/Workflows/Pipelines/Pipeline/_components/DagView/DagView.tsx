...
const DagView: React.FC<DagViewProps> = ({ groupId, pipelineId, tasks }) => {
  ...
  const sidebarTasks = extension?.serviceCustomizations?.workflows?.dagTasks || []
  ...
  const handleEditTask = (task: Workflows.Task) => {
    ...
    if (state.selectedTask) {
      dispatch({type: EnumActionType.SELECT_TASK, payload: state.selectedTask})
    }
    ...
    switch (task.type) {
      case Workflows.EnumTaskType.Function:
        ...
        },
        {
          ...
          onError: () => { dispatch({ type: EnumActionType.DELETE_TASK, payload: task }) }
        })
    }
  }
  ...
  return (
    <div
      id="dag-view"
      className={`${styles["dag-view-container"]}`}
    >
      {
        state.editTask ?
        <TaskEditor 
            ...
            >
              ...
              {deleteTaskIsError && "ERROR:" + deleteTaskError?.message}
              {
                state.tasks.map((task) => {
                  ...
                  let isActive = state.selectedTask && state.selectedTask.id == task.id
                  if (isActive) { classNames += " " + styles["dag-task-active"]}
                  return (
                    <>
                      <Tooltip title={task.description || task.id} placement="right">
                        <div
