...
const PipelineCard: React.FC<PipelineCardProps> = ({groupId, pipeline}) => {
  ...
  return (
    <div className={styles["card"]}>
      <Link className={styles["card-title"]} to={`/workflows/pipelines/${groupId}/${pipeline.id}`}>
        <b>{pipeline.id}</b>
      </Link>
      ...
      <MoreVert className={styles["more"]} onClick={() => {setMenuActive(!menuActive)}}/>
      {
        menuActive && (
          <PipelineCardMenu pipeline={pipeline} groupId={groupId}/>
        )
      }
    </div>
  )
