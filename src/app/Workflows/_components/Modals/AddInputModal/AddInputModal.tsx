        ...
        <div className={styles['form']}>
          <>
            <FormControl variant="standard">
              ...
              <Input
                id="input-name"
                disabled={!!input.name && Object.keys(taskPatch.input || {}).includes(input.name)}
                onChange={(e) => {
                  setInput({...input, name: e.target.value})
                }}
              />
              <FormHelperText>
                The unique identifier for this input. Must be alphanumeric, uppercase, and may contain underscores
              </FormHelperText>
            </FormControl>
            {
            ...
            <>
              <div className={styles['form']}>
                ...
                >
                  ...
                  >
                    {Object.values(Workflows.EnumTaskIOType).map(
                      (type) => {
                        return (
                          <MenuItem
                            value={type}
              ...
              <div className={styles['form']}>
                ...
                >
                        ...
                        task,
                        {
                          input: {
                            ...
                            [input.name!]: {
                              ...
                              value_from: e.target.value !== "literal" ? {[(e.target.value as string)]: ""} : undefined
                            }
                          }
                        }
                      )
                  ...
                  >
                    <MenuItem value={"task_output"} disabled={(taskPatch.depends_on || []).length < 1}>task output{(taskPatch.depends_on || []).length < 1 && " - (available with 1 or more dependencies)"}</MenuItem>
                    <MenuItem value={"env"}>envrionment variable</MenuItem>
                    <MenuItem value={"args"}>pipeline argument</MenuItem>
                    <MenuItem value={"literal"}>-- provide a literal value --</MenuItem>
                  </Select>
                </FormControl>
                <FormHelperText>
                  The source of the input's value. Values can come from pipeline arguments, environment variables, or task outputs. 
                </FormHelperText>
              </div>
                {
                  ...
                  <div className={styles['form']}>
                    {
                      input.source === "task_output" && (task.depends_on || []).length > 0 && (
                        <>
                          <FormControl
                            fullWidth
                            margin="dense"
                            ...
                            >
                              {
                                task.depends_on!.map((dep) => {
                                  return (
                                    <MenuItem value={dep.id}>{dep.id}</MenuItem>
                                  )
                                })
                              }
                            </Select>
