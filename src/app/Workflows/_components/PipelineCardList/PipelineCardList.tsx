  ...
  return (
    <div>
      <div className={styles["cards-container"]}>
        ...
        {
          isLoading ? (
            <div className={`${styles["cards"]} ${styles["skeletons"]} ${styles[colStyle]}`}>
              {
            ...
            <div className={`${styles["cards"]} ${styles[colStyle]}`}>
            {
              pipelines?.result && (
                pipelines.result.map((pipeline, i) => {
                  ...
                  // 6 cards per page
                  i++
                  if (i > cardsPerPage  * page || i <= cardsPerPage * page - cardsPerPage ) {
                    return <></>
                  }
                  return (
                    <PipelineCard pipeline={pipeline} groupId={groupId}/>
                  )
                })
              )
            }
