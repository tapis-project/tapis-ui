...
const JobLaunchModal: React.FC<ToolbarModalProps> = ({ toggle, app }) => {
  const { isLoading, isSuccess, error, reset, submit } =
    Hooks.useSubmit(app.id!, app.version!);
      ...
      <DialogContent>
        {
          guided === undefined && (
            <div style={{display: "flex", flexDirection: "row", gap: "16px"}}>
              <Button variant={"outlined"} onClick={() => {setGuided(true)}} size="large" startIcon={<RocketLaunch/>}>Use guided job launcher</Button>
              <Button variant={"outlined"} onClick={() => {setGuided(false)}} size="large" startIcon={<DataObject/>}>Submit with JSON</Button>
            </div>
          )
        }
        {
