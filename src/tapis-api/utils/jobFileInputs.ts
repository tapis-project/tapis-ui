...
export const getIncompleteAppInputs = (app: Apps.TapisApp): Array<Apps.AppFileInput> => {
  return app.jobAttributes?.fileInputs?.filter(fileInput => !fileInput.sourceUrl) ?? [];
}

export const getIncompleteAppInputsOfType = (app: Apps.TapisApp, inputType: Apps.FileInputModeEnum): Array<Apps.AppFileInput> => {
  return getIncompleteAppInputs(app).filter(
    fileInput => fileInput.inputMode === inputType
  );
}

export const generateRequiredFileInputsFromApp = (app: Apps.TapisApp): Array<Jobs.JobFileInput> => {
  const requiredInputs: Array<Apps.AppFileInput> = getIncompleteAppInputsOfType(app, Apps.FileInputModeEnum.Required);
...
export const fileInputsComplete = (app: Apps.TapisApp, fileInputs: Array<Jobs.JobFileInput>) => {
  ...
  const optionalAppInputs: Array<Apps.AppFileInput> = getIncompleteAppInputs(app).filter(
