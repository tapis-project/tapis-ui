...
describe('ExecSystem job launcher step', () => {
  it('Finds the default logical queue for an app', async () => {
    ...
    expect(getLogicalQueue(tapisApp, [ tapisSystem ], 'testuser2.execution')).toEqual('tapisNormal');
    ...
    expect(getLogicalQueue(tapisApp, [ alternateSystem ], 'alternateSystem')).toEqual('tapisNormal');
    ...
    expect(getLogicalQueue(tapisApp, [ alternateSystem ], 'alternateSystem')).toEqual('alternateQueue');
    ...
    expect(getLogicalQueue(tapisApp, [ alternateSystem ], 'alternateSystem')).not.toBeDefined();
  })
});
