...
const TransferCreate: React.FC<TransferCreateProps> = ({
  ...
  const onSubmit = useCallback(
    ({ tag }) => {
      ...
      create({ elements, tag: tagName }, { onSuccess: () => focusManager.setFocused(true) });
    },
    [destinationSystemId, destinationPath, files, create]
  )
  ...
  return (
    ...
    >
      ...
      <SubmitWrapper
        isLoading={isLoading}
        error={error}
        success={isSuccess ? `Successfully submitted transfer ${data?.result?.tag ??  data?.result?.uuid ?? ''}` : ''}
        reverse
      >
        <Button
          color="primary"
