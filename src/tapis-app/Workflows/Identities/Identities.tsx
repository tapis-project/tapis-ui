...
const Identities: React.FC = () => {
  ...
  return (
    <QueryWrapper isLoading={isLoading} error={error}>
      <h2>Identities <span className={styles["count"]}>{identities.length}</span></h2>
      <Toolbar buttons={["createidentity"]}/>
      <div className={styles["container"]}>
        {identities.length ? identities.map((identity, i) => {
          let evenodd:string = i%2 > 0 ? styles["odd"] : styles["even"]
          let last: string = i == identities.length - 1 ? styles["last"] : ""
          return (
              <Link to={`/workflows/identities/${identity.uuid}`} style={{"textDecoration": "none", "color": "black"}}>
                <div className={`${styles["identity"]} ${evenodd} ${last}`}>
                  <span><Icon name="user"/> {identity.name}</span>
                </div>
              </Link>
          )
        }) : (
