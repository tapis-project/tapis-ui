...
const Groups: React.FC = () => {
  ...
  return (
    <QueryWrapper isLoading={isLoading} error={error}>
      <h2>Groups <span className={styles["count"]}>{groups.length}</span></h2>
      ...
      <div className={styles["container"]}>
        {groups.length ? groups.map((group, i) => {
          ...
          return (
              <Link to={`/workflows/groups/${group.id}`} style={{"textDecoration": "none", "color": "black"}}>
                <div className={`${styles["group"]} ${evenodd} ${last}`}>
                  <span><Icon name="user"/> {group.id}</span>
                </div>
              </Link>
          )
        }) : (
