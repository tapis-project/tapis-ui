...
const S3Archive: React.FC<S3ArchiveProps> = ({archive}) => {
  return (
    <div id={`archive-${archive.id}`}>
      <p><b>id: </b> {archive.id}</p>
      <p><b>type: </b> {archive.type}</p>
      <p><b>owner: </b>{archive.owner}</p>
      <p><b>archiveDir: </b>{archive.archive_dir}</p>
      <p><b>endpoint: </b>{archive.endpoint}</p>
      <p><b>bucket: </b>{archive.bucket}</p>
      <p><b>region: </b>{archive.region}</p>
    </div>
  )
}
...
const TapisSystemArchive: React.FC<TapisSystemArchiveProps> = ({archive}) => {
  return (
    <div id={`archive-${archive.id}`}>
      <p><b>id:</b> {archive.id}</p>
      <p><b>type:</b> {archive.type}</p>
      <p><b>systemId:</b> {archive.system_id}</p>
      <p><b>archiveDir:</b> {archive.archive_dir}</p>
    </div>
  )
}
