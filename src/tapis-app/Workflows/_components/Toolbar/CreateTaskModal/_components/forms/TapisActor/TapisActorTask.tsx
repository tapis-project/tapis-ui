...
const TapisActorTask: React.FC<{ onSubmit: any }> = ({ onSubmit }) => {
  ...
  const [messageDataType, setMessageDataType] = useState<MessageDataType>(MessageDataType.Json)
  const initialValues = {
    id: '',
    description: '',
        ...
        <Form id="newtask-form">
          ...
          { messageDataType == MessageDataType.String ? (
            <FormikInput
              ...
              label="message"
              required={true}
              description={"The string message you want to the Tapis actor to consume.`"}
              aria-label="Input"
              value={message}
            />
          ) : (
