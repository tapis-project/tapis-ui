...
const NotificationToast = () => {
  type TransitionType = React.ComponentType<SlideProps & {
    children?: React.ReactElement<any, any> | undefined;
  }> | undefined
  const { notifications, markread } = useNotifications();
  const [open, setOpen] = useState(false);
  const [notificationRecord, setNotificationRecord] = useState<NotificationRecord | null>(null);
  ...
  type HandleCloseType = (event: React.SyntheticEvent<any, Event>, reason: SnackbarCloseReason) => void;
  ...
  return (
    notificationRecord && !notificationRecord.read
      ? <Snackbar
...
export const ToastMessage: React.FC<{ notification: Notification }> = ({ notification }) => {
  return (
    <>
