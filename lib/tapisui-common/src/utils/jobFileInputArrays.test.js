...
import { getIncompleteAppInputArraysOfType, generateRequiredFileInputArraysFromApp, fileInputArraysComplete, getIncompleteJobInputArrays, getAppInputArraysIncludedByDefault, } from './jobFileInputArrays';
describe('Job File Input Arrays utils', function () {
    it('finds incomplete input arrays of a specific kind', function () {
        expect(getIncompleteAppInputArraysOfType(tapisApp, Apps.FileInputModeEnum.Required)).toEqual([
            {
    ...
    it('generates required file inputs from incomplete app required file input arrays', function () {
    ...
    it('detects a complete job request that satisfies an app with fully formed required input arrays', function () {
        expect(fileInputArraysComplete(tapisApp, [
            { name: 'required-incomplete', sourceUrls: ['file2.txt'] },
        ])).toEqual(true);
    });
    it('detects an incomplete job request with required inputs lacking sourceUrls', function () {
        // Remove a required sourceUrl from the app fixture
        expect(fileInputArraysComplete(tapisApp, [])).toEqual(false);
        expect(fileInputArraysComplete(tapisApp, [{ name: 'required-incomplete' }])).toEqual(false);
        expect(fileInputArraysComplete(tapisApp, [
    ...
    it('detects an incomplete job request with optional inputs lacking a source url', function () {
        ...
        modifiedApp.jobAttributes.fileInputArrays =
            (_c = (_b = (_a = modifiedApp.jobAttributes) === null || _a === void 0 ? void 0 : _a.fileInputArrays) === null || _b === void 0 ? void 0 : _b.filter(function (input) { return input.inputMode !== Apps.FileInputModeEnum.Required; })) !== null && _c !== void 0 ? _c : [];
        ...
        expect(fileInputArraysComplete(modifiedApp, [{ name: 'optional-incomplete' }])).toEqual(false);
        // A job that includes an complete OPTIONAL file input should be fine
        expect(fileInputArraysComplete(modifiedApp, [
            {
    ...
    it('getAppInputsIncludedByDefault', function () {
        ...
        expect(getAppInputArraysIncludedByDefault([
            {
                name: 'Required',
                sourceUrls: ['required.txt'],
                targetDir: 'required',
